...
df_continuous$LVAD <- as.factor(df_continuous$LVAD)
...
df_continuous_copy <- df_continuous
...
### 6-Status
```{r, warning=F, echo=F, message=F, comment=NA}
sixstatus_c <- df_continuous[ ,c('PX_ID', 'death6week', 't_start', 't_stop')]
...
### French-CRS
```{r, warning=F, echo=F, message=F, comment=NA}
model1_c <- df_continuous[ ,c('PX_ID', 'death6week', 't_start', 't_stop')]
model1_c$death6week <- as.numeric(as.character(model1_c$death6week))

model1_cpredict <- predict(model1, newdata = df_continuous, type = 'response')
...
### US-CRS
```{r, warning=F, echo=F, message=F, comment=NA}
model6_c <- df_continuous[ ,c('PX_ID', 'death6week', 't_start', 't_stop')]
model6_c$death6week <- as.numeric(as.character(model6_c$death6week))


model6_cpredict <- predict(model6, newdata = df_continuous, type = 'response')
...
## Comparison of Training and Testing Datasets (Table 1)
...
df_continuous_table1 <- df %>%
  select(PX_ID, status, age, albumin, bilirubin, creatinine, sodium, BNP, PASP, PADP, central_venous_pressure,
         systolicBP, diastolicBP, cardiac_output) %>%
  group_by(PX_ID) %>%
  filter(row_number() == 1)
df_continuous_table1$train_test <- 'Train'
df_continuous_table1$train_test[df_continuous_table1$PX_ID %in% test_ids] <- 'Test'

df_cand_thor <- haven::read_sas('./cand_thor.sas7bdat')
df_cand_thor <- df_cand_thor %>% select(PX_ID, CAN_GENDER, CAN_RACE) %>% unique()
df_continuous_table1 <- merge(df_continuous_table1, df_cand_thor, by='PX_ID', all.x=T, all.y=F)
...
model6_allpredict <- predict(model6, newdata = df_continuous_copy)
model6_allpredict <- model6_allpredict[!is.na(model6_allpredict)]


cutoffs <- quantile(model6_allpredict, probs = seq(0, 1, length.out = 50))
...
## Plot 50-Point Score Based on 1-Year RMST for US-CRS model
```{r, warning=F, echo=F, message=F, comment=NA, fig.align='center', fig.height=8, fig.width=8}
model6_rmst <- model6_c
...
model6_rmst <- model6_rmst %>% 
  ...
  mutate(product = time_diff * (1-model6_cpredict)) 
...
## Plot calibration for US-CRS
```{r, warning=F, echo=F, message=F, comment=NA, fig.align='center', fig.height=8, fig.width=8}
model6_calibration <- df_intervals_test_model6 %>%
  select(PX_ID, death6week, status) %>%
  mutate(predicted = model6_predict)

model6_calibration %>%
  mutate(decile_US_CRS = ntile(predicted, 10)) %>%
  group_by(decile_US_CRS) %>%
  summarise(observed = mean(as.numeric(as.character(death6week))),
...
```{r, warning=F, echo=F, message=F, comment=NA, fig.align='center', fig.height=8, fig.width=8}
model6_calibration <- subset(model6_calibration, !is.na(model6_calibration$status))

model6_calibration %>%
  ggplot(aes(y = predicted, x = status, color = factor(status))) +
  geom_boxplot() +
