...
library(pacman)
pacman::p_load(dplyr, splines, knitr, kableExtra, glmnet, caret, gtsummary, pROC, xgboost, Ckmeans.1d.dp, forestplot, survival, DescTools)
...
mat_test_all_numeric <- mat_test_all_numeric[ , c('(Intercept)', 
                                                  ...
                                                  'IABP', 'ECMO', 'LVAD', 'MCSD_Complication', 'SystolicBP',
                                                  'DiastolicBP', 'PASP', 'PADP', 'Heart_Rate10', 'Cardiac_Output',
                                                  'Central_Venous_Pressure', 'Arterial_Lactate', 'CPO', 'API',
                                                  'PAPI', 'Hemoglobin',
...
DMat_test <- xgb.DMatrix(as.matrix(mat_test_numeric[,c('(Intercept)', 
                                                       ...
                                                       'Arterial_Lactate', 'CPO', 'API', 'PAPI', 'Hemoglobin',
                                                       'Albumin', 'Bilirubin', 'eGFR', 'Sodium', 'AST100', 'BNP100',
                                                       'BUN10', 'INR', 'Life_Arrhythmia', 'BiVAD_NoDischarge',
...
colnames(DMat_test) <- c('(Intercept)', 'Interval_1', 'Interval_2', 'Interval_3', 'Interval_4',
                         ...
                         'Arterial_Lactate', 'CPO', 'API', 'PAPI', 'Hemoglobin', 'Albumin',
                         'Bilirubin', 'eGFR', 'Sodium', 'AST100', 'BNP100', 'BUN10', 'INR',
...
## Calculate Harrell's C-Indices
```{r, warning=F, echo=F, message=F, comment=NA}
sixstatus_c <- df_intervals_test[ ,c('death6week', 'interval_start', 'interval_stop', 'status')]
sixstatus_c$status_reverse <- 6 - as.numeric(sixstatus_c$status) # Since lower status is worse mortality
sixstatus_c$death6week <- as.numeric(as.character(sixstatus_c$death6week))


concordance(coxph(Surv(interval_start, interval_stop, death6week) ~ status_reverse, data = sixstatus_c), timewt = 'n')
```


```{r, warning=F, echo=F, message=F, comment=NA}
model1_c <- df_intervals_test_model1[ ,c('death6week', 'interval_start', 'interval_stop')]
model1_c$death6week <- as.numeric(as.character(model1_c$death6week))
model1_c$model1_predict <- model1_predict


concordance(coxph(Surv(interval_start, interval_stop, death6week) ~ model1_predict, data = model1_c), timewt = 'n')
```


```{r, warning=F, echo=F, message=F, comment=NA}
model6_c <- df_intervals_test_model6[ ,c('death6week', 'interval_start', 'interval_stop')]
model6_c$death6week <- as.numeric(as.character(model6_c$death6week))
model6_c$model6_predict <- model6_predict


concordance(coxph(Surv(interval_start, interval_stop, death6week) ~ model6_predict, data = model6_c), timewt = 'n')
```


## Calculate Brier score
```{r, warning=F, echo=F, message=F, comment=NA}
DescTools::BrierScore(model1_c$death6week,
                      model1_c$model1_predict)
```


```{r, warning=F, echo=F, message=F, comment=NA}
DescTools::BrierScore(model6_c$death6week,
                      model6_c$model6_predict)
```
