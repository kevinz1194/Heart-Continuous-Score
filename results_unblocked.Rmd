...
<!-- 8. Make prediction figures on unblocked dataset (Line 543) -->
...
model_cox6 <- coxph(Surv(t_start, t_stop, death) ~ 
                    status_initial, 
                  data = df[df$train_test == 'Train',])
eta6 <- model_cox6$linear.predictor

survival.time6 <- unlist(as.vector(df[df$train_test == 'Train' & !is.na(df$status_initial),
                                      't_stop']))
survival.status6 <- unlist(as.vector(df[df$train_test == 'Train'& !is.na(df$status_initial),
                                        'death']))


risksetROC::risksetAUC(Stime = survival.time, status = survival.status,
                       marker = eta, method = 'Cox', tmax = max(survival.time))
risksetROC::risksetAUC(Stime = survival.time6, status = survival.status6,
                       marker = eta6, method = 'Cox', tmax = max(survival.time6))
```
