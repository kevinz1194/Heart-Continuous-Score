...
df$status_ranking <- 10 - as.numeric(df$status)
df <- df %>% 
...
### Functions to bootstrap standard errors
bootstrap_six <- replicate(2000, get_overall_six(df[sample(nrow(df), nrow(df), replace = T), ]))
bootstrap_french <- replicate(2000, get_overall_french(df[sample(nrow(df), nrow(df), replace = T), ]))
bootstrap_us <- replicate(2000, get_overall_us(df[sample(nrow(df), nrow(df), replace = T), ]))
...
```{r, warning=F, message=F, comment=NA}
get_overall_six(df)
quantile(bootstrap_six, c(0.025, 0.975))
```

```{r, warning=F, message=F, comment=NA}
get_overall_french(df)
quantile(bootstrap_french, c(0.025, 0.975))
```

```{r, warning=F, message=F, comment=NA}
get_overall_us(df)
quantile(bootstrap_us, c(0.025, 0.975))
```
