...
<!-- 1. Set up original modified dataset (Line 36) -->
<!-- 2. Merge with status justification datasets and create variables (Line 154) -->
...
<!-- 5. Add in BNP type (Line 839) -->
<!-- 6. Create intervals (Line 856) -->
<!-- 7. Add memory variables (Line 1113) -->
<!-- 8. Death in 6 weeks (Line 1255) -->
<!-- 9. Impute variables (Line 1292) -->
<!-- 10. Generate new clinical variables (Line 1457) -->
<!-- 11. Sample checks between blocked and unblocked (Line 1505) -->
...
df_riskstrat <- df_riskstrat %>% select(px_id, HrSevFailNtBnpType) %>%
  filter(HrSevFailNtBnpType != '')

colnames(df_riskstrat) <- c('PX_ID', 'BNP_Type')


df <- merge(df, df_riskstrat, by = 'PX_ID', all.x = T, all.y = F)
```
...
for(i in nrow(df_intervals):2) {
  if(df_intervals$PX_ID[i] == df_intervals$PX_ID[i-1] & df_intervals$interval_start[i] > df_intervals$max_time[i]) {
    ...
    df_intervals$BNP_Type[i] <- df_intervals$BNP_Type[i-1]
...
for(j in 1:14) {
  for(i in 1:(nrow(df_intervals)-j)) {
    if(df_intervals$interval_start[i] <= df_intervals$t_start[i+j] & 
      ...
      df_intervals$BNP_Type[i] <- df_intervals$BNP_Type[i+j]
...
df_intervals$BNP[is.na(df_intervals$BNP) & df_intervals$BNP_Type == 'BNP'] <- 
  median(df_intervals$BNP[df_intervals$BNP_Type == 'BNP'], na.rm=T)
df_intervals$BNP[is.na(df_intervals$BNP) & df_intervals$BNP_Type == 'NT Pro BNP'] <- 
  median(df_intervals$BNP[df_intervals$BNP_Type == 'NT Pro BNP'], na.rm=T)
df_intervals$BNP[is.na(df_intervals$BNP) & is.na(df_intervals$BNP)] <- 
  median(df_intervals$BNP[is.na(df_intervals$BNP)], na.rm=T)
...
# 10. Generate new clinical variables
...
# 11. Sample checks between blocked and unblocked
