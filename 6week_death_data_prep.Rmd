...
<!-- 1. Set up original modified dataset (Line 31) -->
<!-- 2. Merge with status justification datasets and create variables (Line 145) -->
<!-- 3. Merge with cand_thor dataset and create create time-related variables (Line 477) -->
<!-- 4. Set up remaining clinical variables (Line 706) -->
<!-- 5. Create intervals (Line 802) -->
<!-- 6. Add memory variables (Line 1027) -->
<!-- 7. Death in 6 weeks (Line 1170) -->
<!-- 8. Impute variables (Line 1205) -->
<!-- 9. Generate new clinical variables (Line 1382) -->
...
colnames(df) <- c('PX_ID', 'JustId', 'events', 'date', 't_start', 't_stop', 
                  'systolicBP', 'PCWP', 'central_venous_pressure', 'cardiac_output', 'arterial_lactate', 'AST',
                  'creatinine', 'bilirubin', 'albumin', 'sodium', 'BNP', 'heart_rate', 
                  'diastolicBP', 'PASP', 'PADP', 'hemoglobin', 'LDH', 'BUN',
                  'INR', 'diagnosis', 'VAD', 'VAD_brand', 'diabetes', 'dialysis', 
                  'age_m', 'peripheral_vascular','status_criteria', 'status', 'exception', 'ecmo_hemo',
                  'ecmo_without_hemo', 'bivad', 'iabp_hemo', 'iabp_cardiacindex', 'iabp_without_hemo', 'InoSingle',
                  
                  'InoMultiple', 'InoSinDobutamine', 'InoSinMilrinone', 
                  'InoSinEpinephrine', 'InoMulDobutamine', 'InoMulMilrinone',
                  
                  'InoMulEpinephrine', 'InoMulDopamine', 'ExtInoSingleDose', 
                  'ExtInoMultiDose', 'ExtInoSinDobutamine', 'ExtInoSinMilrinone', 
                  
                  'ExtInoSinEpinephrine', 'ExtInoMulDobutamine', 'ExtInoMulMilrinone', 
                  'ExtInoMulEpinephrine', 'ExtInoMulDopamine', 'ExtMcsdDobutamine',
                  
                  'ExtMcsdDopamine', 'ExtMcsdEpinephrine', 'ExtMcsdMilrinone', 
                  'ExtInotropeDobutamine', 'ExtInotropeMilrinone', 'ExtInotropeEpinephrine', 
                  
                  'ExtInotropeDopamine', 'TAH',
                  'dobutamine_dose', 'milrinone_dose', 'dopamine_dose', 'epinephrine_dose')
...
## Read in datasets
df_just <- haven::read_sas('./JustFormHR.sas7bdat')
...
rm(df_just, df_just1, df_just2, df_just3, df_just4)
```



## Inotrope variables
These lab variables are categorical, with cutoffs based on the 'High' cutoffs in the data dictionary. \newline
...
df <- df %>%
  mutate(dopamine = case_when(
    ...
    as.numeric(dopamine_dose) >= 3 | !is.na(McsdWithRhfDopamine) | as.numeric(InotropeDopamine) >= 3 |
    !is.na(ExtMcsdDopamine.y) | as.numeric(ExtInotropeDopamine.y) >= 3 | 
...
df <- df %>% 
  mutate(dobutamine = case_when(
    ...
    as.numeric(dobutamine_dose) >= 7.5 | !is.na(McsdWithRhfDobutamine) | as.numeric(InotropeDobutamine) >= 7.5 |
    !is.na(ExtMcsdDobutamine.y) | as.numeric(ExtInotropeDobutamine.y) >= 7.5 | 
...
df <- df %>% 
  mutate(milrinone = case_when(
    ...
    as.numeric(milrinone_dose) >= 0.5 | !is.na(McsdWithRhfMilrinone) | as.numeric(InotropeMilrinone) >= 0.5 |
    !is.na(ExtMcsdMilrinone.y) | as.numeric(ExtInotropeMilrinone.y) >= 0.5 | 
...
df <- df %>% 
  mutate(epinephrine = case_when(
    ...
    as.numeric(epinephrine_dose) >= 0.02 | !is.na(McsdWithRhfEpinephrine) | as.numeric(InotropeEpinephrine) >= 0.02 |
    !is.na(ExtMcsdEpinephrine.y) | as.numeric(ExtInotropeEpinephrine.y) >= 0.02 | 
...
df_candthor <- df_candthor %>% 
  ...
  ## Extra time after last active date in days (can be 0); cannot be transplanted or dead at removal
  mutate(
...
paste0('Difference Between Candthor and Dataset: ', setdiff(transplant_ids_candthor, transplant_ids))
...
df$t_stop[df$last_group == '1' & df$transplant == '0' & !is.na(df$removal_time) & df$t_stop != df$removal_time] <-
  as.numeric(df$removal_time[df$last_group == '1' & 
                               df$transplant == '0' & 
                               !is.na(df$removal_time) & df$t_stop != df$removal_time])

### We only replace with death date if extra time is 0
df$t_stop[df$last_group == '1' & !is.na(df$death_time) & df$extra_time == 0 & df$death_time > df$t_stop] <-
  as.numeric(df$death_time[df$last_group == '1' & !is.na(df$death_time) & df$extra_time == 0 & 
...
df$t_stop[df$last_group == '1' & df$transplant == '1' & df$transplant_time != df$t_stop] <-
  as.numeric(df$transplant_time[df$last_group == '1' & df$transplant == '1' & df$transplant_time != df$t_stop])

```



### Sample PX_IDs showing the change; now correct start and stop times accordingly
```{r, warning=F, comment=NA, message=F}
### Extra time exists; we need 'phantom' intervals
...
```{r, warning=F, comment=NA, message=F}
### Extra time is greater than 42; we need to cut off the intervals
...
df$interval_cut_flag[df$last_group == '1' & df$transplant == '0' &
                       (df$extra_time == 0 | df$extra_time > 42) &
...
df <- df %>% group_by(PX_ID) %>%  
  ...
  mutate(
    t_stop = case_when(
      last_group == '1' & transplant == '0' & (extra_time == 0 | extra_time > 42) & 
...
paste0('Difference Between Candthor and Dataset: ', setdiff(candthor_death_ids, death_ids))
...
```{r, warning=F, comment=NA, message=F}
duplicate_counts <- ceiling(df$time / 14)
duplicate_counts[duplicate_counts == 0] <- 1
row_reps <- rep(1:nrow(df), duplicate_counts)
...
### Find extra intervals and fill in previous row with data
```{r, warning=F, comment=NA, message=F}
...
## Ignore 0-day intervals
df_intervals <- subset(df_intervals, df_intervals$interval_start <= df_intervals$max_time)



rm(duplicate_counts, row_reps); invisible(gc())
...
df_intervals <- df_intervals %>%
  ...
  fill(IV_inotropes_ever, .direction = 'down') %>%
  fill(IABP_ever, .direction = 'down') %>%
...
## Includes exception as well
df_intervals$exception <- df_intervals$exception_ever
df_intervals <- df_intervals %>%
  select(-c(exception_ever))
```
...
# If transplanted, no death within 6 weeks
...
df_intervals$BNP[is.na(df_intervals$BNP) & df_intervals$status == '1'] <-
  median(df_intervals$BNP[df_intervals$status == '1'], na.rm=T)
df_intervals$BNP[is.na(df_intervals$BNP) & df_intervals$status == '2'] <-
  median(df_intervals$BNP[df_intervals$status == '2'], na.rm=T)
df_intervals$BNP[is.na(df_intervals$BNP) & df_intervals$status == '3'] <-
  median(df_intervals$BNP[df_intervals$status == '3'], na.rm=T)
df_intervals$BNP[is.na(df_intervals$BNP) & df_intervals$status == '4'] <-
  median(df_intervals$BNP[df_intervals$status == '4'], na.rm=T)
df_intervals$BNP[is.na(df_intervals$BNP) & df_intervals$status == '5'] <-
  median(df_intervals$BNP[df_intervals$status == '5'], na.rm=T)
df_intervals$BNP[is.na(df_intervals$BNP) & df_intervals$status == '6'] <-
  median(df_intervals$BNP[df_intervals$status == '6'], na.rm=T)
```


### Impute status 4 and 6 missings
```{r, warning=F, comment=NA, message=F}
df_intervals <- df_intervals %>%
...
df_intervals$central_venous_pressure[is.na(df_intervals$central_venous_pressure) & 
                                       df_intervals$status == '4' & df_intervals$LVAD == '1'] <- 10
df_intervals$PASP[is.na(df_intervals$PASP) & df_intervals$status == '4' & df_intervals$LVAD == '1'] <- 35
df_intervals$PADP[is.na(df_intervals$PADP) & df_intervals$status == '4' & df_intervals$LVAD == '1'] <- 15
df_intervals$PCWP[is.na(df_intervals$PCWP) & df_intervals$status == '4' & df_intervals$LVAD == '1'] <- 15
df_intervals$systolicBP[is.na(df_intervals$systolicBP) & df_intervals$status == '4' & 
                          df_intervals$LVAD == '1'] <- 100
df_intervals$diastolicBP[is.na(df_intervals$diastolicBP) & df_intervals$status == '4' & 
                           df_intervals$LVAD == '1'] <- 80
df_intervals$heart_rate[is.na(df_intervals$heart_rate) & df_intervals$status == '4' & 
                          df_intervals$LVAD == '1'] <- 8.5


df_intervals$central_venous_pressure[is.na(df_intervals$central_venous_pressure) & df_intervals$status == '6'] <- 12
df_intervals$PASP[is.na(df_intervals$PASP) & df_intervals$status == '6'] <- 38
df_intervals$PADP[is.na(df_intervals$PADP) & df_intervals$status == '6'] <- 18
df_intervals$PCWP[is.na(df_intervals$PCWP) & df_intervals$status == '6'] <- 18
df_intervals$systolicBP[is.na(df_intervals$systolicBP) & df_intervals$status == '6'] <- 110
df_intervals$diastolicBP[is.na(df_intervals$diastolicBP) & df_intervals$status == '6'] <- 80
df_intervals$heart_rate[is.na(df_intervals$heart_rate) & df_intervals$status == '6'] <- 8.5
```


### Final cleanup of variables
```{r, warning=F, comment=NA, message=F}
### Reupdate min/max variables based on imputations
...
### Unmatched case is removed (1 patient)
```{r, warning=F, comment=NA, message=F}
df_intervals %>%
  filter(is.na(age))
...
### Temporary surgery MCS
```{r, warning=F, comment=NA, message=F}
df_intervals <- df_intervals %>%
  mutate(temp_surg_MCS = case_when(
    ECMO == '1' | temp_surg_LVAD == '1' | BiVAD_no_discharge == '1' ~ 1,
    TRUE ~ 0))
```
...
df_intervals$kidney_deficit <- pmax(100-df_intervals$eGFR, 0)
```


```{r, warning=F, comment=NA, message=F}
df_intervals %>%
  select(PX_ID, creatinine, CAN_GENDER, dialysis, eGFR, kidney_deficit)
```



```{r, warning=F, comment=NA, message=F}
df_intervals$last_group <- NULL
