...
<!-- 1. Set up original modified dataset (Line 41) -->
<!-- 2. Merge with status justification datasets and create variables (Line 173) -->
<!-- 3. Merge with cand_thor dataset and create create time-related variables (Line 544) -->
<!-- 4. Set up remaining clinical variables (Line 805) -->
<!-- 5. Add in BNP type and fix sodium (Line 917) -->
<!-- 6. Create intervals (Line 938) -->
<!-- 7. Add memory variables (Line 1266) -->
<!-- 8. Death in 6 weeks (Line 1408) -->
<!-- 9. Count missingness (Line 1444) -->
<!-- 10. Impute variables (Line 1517) -->
<!-- 11. Generate new clinical variables (Line 1669) -->
<!-- 12. Histograms of variables (Line 1718) -->
<!-- 13. Sample checks between blocked and unblocked (Line 1800) -->
...
library(pacman)
pacman::p_load(tidyverse, Hmisc, ggplot2, expss, haven, lubridate, 
               knitr, cowplot)
...
colnames(df) <- c('PX_ID', 'JustId', 'events', 'date', 
                  ...
                  'TAH','dobutamine_dose', 'milrinone_dose', 
...
df_candthor <- df_candthor %>% 
  ...
  ## Extra time after removal date in days (can be 0 or negative if no death); 
  ## cannot be transplanted or dead at removal
...
df %>% filter(PX_ID == '1252414' | PX_ID == '1484214' | PX_ID == '1504645') %>% 
  select(PX_ID, t_start, t_stop, last_group, removal_time, death_time, 
         extra_time, transplant, transplant_time)
```
...
df <- df %>% 
  mutate(death_flag = case_when(
    (!is.na(death_date_max) | removal_code_cand_thor == '8') & 
      transplant == '0' & extra_time <= 42 ~ '1' ,
    TRUE ~ '0'))
```
...
df_candthor_raw %>% filter(PX_ID == '1288472') %>%
  select(PX_ID, CAN_LISTING_DT, CAN_DEATH_DT, PERS_SSA_DEATH_DT, 
         PERS_OPTN_DEATH_DT, REC_TX_DT, CAN_REM_CD)
...
```{r, warning=F, comment=NA, message=F}
save(df, file = './intervals_v18_continuous.RData')
```
...
for (i in 1:(nrow(df_intervals))) {
  ...
  if (df_intervals$interval_start[i] == 0) {
    ...
    df_intervals$durable_LVAD[i] <- initial_values %>% pull(durable_LVAD)
    ...
    df_intervals$durable_LVAD[i] <- final_values %>% pull(durable_LVAD)
    ...
    df_intervals$durable_LVAD[i] <- previous_values %>% pull(durable_LVAD)
...
df_candthor %>%
  ...
  select('PX_ID', 'CAN_LISTING_DT',
         ...
         'CAN_AGE_IN_MONTHS_AT_LISTING', 'CAN_DGN', 'CAN_DIAB_TY', 
         'CAN_DIAL', 'CAN_INIT_STAT') %>%
  arrange(PX_ID)
```
...
```{r, warning=F, comment=NA, message=F}
save(df_intervals, file = './intervals_v18_impute_6wkdeath.RData')
```
...
```{r}
df_intervals_sample %>%
  select(PX_ID, interval_start, interval_stop, last_group, status, systolicBP, 
         PCWP, cardiac_output, BNP)
```
