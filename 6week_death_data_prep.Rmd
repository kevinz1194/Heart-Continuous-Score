...
<!-- 6. Create intervals (Line 857) -->
<!-- 7. Add memory variables (Line 1114) -->
<!-- 8. Death in 6 weeks (Line 1256) -->
<!-- 9. Impute variables (Line 1293) -->
<!-- 10. Generate new clinical variables (Line 1456) -->
<!-- 11. Sample checks between blocked and unblocked (Line 1504) -->
...
for(i in nrow(df_intervals):2) {
  if(df_intervals$PX_ID[i] == df_intervals$PX_ID[i-1] & df_intervals$interval_start[i] > df_intervals$max_time[i]) {
    ...
    df_intervals$BNP_NT_Pro[i] <- df_intervals$BNP_NT_Pro[i-1]
...
for(j in 1:14) {
  for(i in 1:(nrow(df_intervals)-j)) {
    if(df_intervals$interval_start[i] <= df_intervals$t_start[i+j] & 
      ...
      df_intervals$BNP_NT_Pro[i] <- df_intervals$BNP_NT_Pro[i+j]
...
df_intervals <- df_intervals %>%
  mutate(eGFR = case_when(
    ...
    CAN_GENDER == 'F' & !is.na(creatinine) ~ 142 * (pmin((creatinine / 0.7), 1)^(-0.241)) *
      (pmax((creatinine / 0.7), 1)^(-1.2)) * 0.9938^(age*10) * 1.012,
    
    CAN_GENDER == 'M' & !is.na(creatinine) ~ 142 * (pmin((creatinine / 0.9), 1)^(-0.302)) *
      (pmax((creatinine / 0.7), 1)^(-1.2)) * 0.9938^(age*10)))
