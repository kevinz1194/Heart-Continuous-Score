...
<!-- 7. Score conversion possibilities (Line 399) -->
...
colnames(df_unblocked) <- 
  c('PX_ID', 't_start', 't_stop', 'status', 'last_group', 'systolicBP', 
...
df_unblocked %>%
  ungroup() %>%
  mutate(decile_US_CRS = ntile(model6_predict, 10)) %>%
  group_by(decile_US_CRS) %>%
  summarise(observed = mean(as.numeric(as.character(death))),
            model6_predict = mean(model6_predict)) %>%
