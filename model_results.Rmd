...
<!-- 1. Set up dataset with generated cardiac variables (Line 39) -->

<!-- 2. Set up matrices for predictions (Line 152) -->
...
<!-- 5. Obtain machine learning model results (Line 1768) -->

<!-- 6. Make predictions on test dataset (Line 1830) -->

<!-- 7. Create AUC curves (Line 1891) -->

<!-- 8. Variable importance plots from gradient-boosted models (Line 1947) -->

<!-- 9. Create calibration plot in test dataset (Line 2008) -->

<!-- 10. Create facet plot of 50 point score at initial listing (Line 2065) -->

<!-- 11. Examine gender and race differences in prediction (Line 2097) -->

<!-- 12. Check for informative censoring (Line 2144) -->

<!-- 13. Examine a model with center random effects (Line 2179) -->

<!-- 14. Examine model performance based on temporal split (Line 2196) -->
...
### US-CRS with 2-week survival window

```{r}

df_intervals <- df_intervals %>% 
  group_by(PX_ID) %>% 
  mutate(death2week = if_else(death6week == 1 & last_group == 1, 1, 0)) %>% 
  ungroup()

model_final_2wk <- glm(formula = as.formula(
  death2week ~ 
    albumin + log(bilirubin + 1) + 
    eGFR + sodium + 
    (log(BNP + 1):BNP_NT_Pro) + 
...
model_final_2wk_results <- as.data.frame(summary(model_final_2wk)$coefficients)
model_final_2wk_results$name <- rownames(model_final_2wk_results)

model_final_2wk_results <- model_final_2wk_results[ ,c('name', 'Estimate', 'Pr(>|z|)')]
colnames(model_final_2wk_results) <- c('Variable', 'Estimate', 'PValue')
rownames(model_final_2wk_results) <- seq(1:nrow(model_final_2wk_results))
model_final_2wk_results <- model_final_2wk_results %>% 
  mutate(Estimate = round(Estimate, 3),
           PValue = round(PValue, 3))

model_final_2wk_results$Variable <- c('Intercept',  
                                  'Albumin', 'Log Bilirubin+1',
                                  'eGFR', 'Sodium', 
                                  'LVAD', 'Short MCS Ever',
                                  'Log BNP+1, Regular BNP',
...
model_final_2wk_results %>%
  kable(align=c('c', 'c', 'c')) %>%
  kable_styling(bootstrap_options='striped', full_width=F, position='center') %>%
  column_spec(1, border_right=T)

df_intervals_train_2wk <- df_intervals[df_intervals$train_test == 'Train',]

 auc(df_intervals_train_2wk$death2week ~ predict(model_final_2wk, 
                          newdata = df_intervals_train_2wk,
...
df_intervals_test_calib <- 
  ...
  dplyr::summarise(predicted = mean(uscrs_predict),
...
df_gender_race %>%
  # filter(PX_ID %in% c(female_ids, male_ids)) %>%
  tbl_summary(by = female, 
              include = c('uscrs_predict', 'uscrs_score'),
              digits = all_continuous() ~ 5) %>%
  add_p()

df_gender_race %>%
  # filter(PX_ID %in% c(white_ids, nonwhite_ids)) %>%
  tbl_summary(by = race, 
              include = c('uscrs_predict', 'uscrs_score'),
              digits = all_continuous() ~ 5) %>%
  add_p()
