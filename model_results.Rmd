...
<!-- 1. Set up dataset with generated cardiac variables (Line 34) -->
<!-- 2. Set up matrices for predictions (Line 138) -->
<!-- 3. Run selected logistic regression models and create coefficient plots (Line 594) -->
<!-- 4. Obtain machine learning model results (Line 1243) -->
<!-- 5. Make predictions on test dataset (Line 1301) -->
<!-- 6. Create AUC curves (Line 1348) -->
<!-- 7. Variable importance plots from gradient-boosted models (Line 1430) -->
<!-- 8. Create calibration plot in test dataset (Line 1491) -->
<!-- 9. Create facet plot of 50 point score at initial listing (Line 1539) -->
...
library(pacman)
pacman::p_load(tidyverse, splines, knitr, kableExtra, glmnet, caret, gtsummary, pROC, xgboost, survival, DescTools, cowplot)
...
model2 <- glm(formula = as.formula(
  death6week ~ 
    short_MCS_French_new + log(bilirubin + 1) + eGFR + log(BNP + 1)), 
  data = df_intervals[df_intervals$train_test == 'Train',], 
  family = binomial)
...
model2_results$Variable <- c('Intercept', 'Short Term MCS (New)', 'Log Bilirubin+1', 'eGFR', 'Log BNP+1')
...
save(model2, file = './model_frenchCRS_new.RData')
```
...
model3_results$Variable <- c('Intercept', 'PCWP', 'Albumin', 'Log Bilirubin+1', 
                             'eGFR', 'Sodium', 
                             'Log BNP+1', 'NT-proBNP',
                             'LVAD', 'Short MCS Ever', 'NT-proBNP Interaction')
...
### Selected Model #4: US candidate risk score replacing PCWP
...
model4_results$Variable <- c('Intercept', 'CPO', 'API', 'Albumin', 'Log Bilirubin+1', 'eGFR', 
                             'Sodium', 'Log BNP+1', 'NT-proBNP',
                             'LVAD', 'Short MCS Ever', 'NT-proBNP Interaction')
...
model5_results$Variable <- c('Intercept', 'CPO', 'Albumin', 'Log Bilirubin+1', 'eGFR', 'Sodium', 
                             'Log BNP+1', 'NT-proBNP',
                             'LVAD', 'Short MCS Ever', 
                             'NT-proBNP Interaction')
...
model6_results$Variable <- c('Intercept', 'API', 'Albumin', 'Log Bilirubin+1', 
                             'eGFR', 'Sodium',                          
                             'Log BNP+1', 'NT-proBNP',
                             'LVAD', 'Short MCS Ever', 
                             'NT-proBNP Interaction')
...
model7_results$Variable <- c('Intercept', 'CPO', 'PAPi', 'Albumin', 'Log Bilirubin+1',
                             'eGFR', 'Sodium', 
                             'Log BNP+1', 'NT-proBNP',
                             'LVAD', 'Short MCS Ever', 
                             'NT-proBNP Interaction')
...
```{r, echo=F, warning=F, comment=NA, fig.align='center'}
paste0('Pearson correlation ', cor(df_intervals$api, df_intervals$papi, use = 'pairwise.complete.obs', method = 'pearson'))
paste0('Spearman correlation ', cor(df_intervals$api, df_intervals$papi, use = 'pairwise.complete.obs', method = 'spearman'))
```
...
model8_results <- as.data.frame(summary(model8)$coefficients)
model8_results$name <- rownames(model8_results)

model8_results <- model8_results[ ,c('name', 'Estimate', 'Pr(>|z|)')]
colnames(model8_results) <- c('Variable', 'Estimate', 'PValue')
rownames(model8_results) <- seq(1:nrow(model8_results))
model8_results <- model8_results %>% 
  mutate(Estimate = round(Estimate, 3),
           PValue = round(PValue, 3))

model8_results$Variable <- c('Intercept', 'PAPi', 'Albumin', 'Log Bilirubin+1', 
                             'eGFR', 'Sodium', 
                             'Log BNP+1', 'NT-proBNP',
                             'LVAD', 'Short MCS Ever', 
                             'NT-proBNP Interaction')
...
model9_results <- as.data.frame(summary(model9)$coefficients)
model9_results$name <- rownames(model9_results)

model9_results <- model9_results[ ,c('name', 'Estimate', 'Pr(>|z|)')]
colnames(model9_results) <- c('Variable', 'Estimate', 'PValue')
rownames(model9_results) <- seq(1:nrow(model9_results))
model9_results <- model9_results %>% 
  mutate(Estimate = round(Estimate, 3),
           PValue = round(PValue, 3))

model9_results$Variable <- c('Intercept', 'CPO', 'PAPi', 'API', 'AST',
...
```{r, echo=F, warning=F, comment=NA, fig.align='center'}
model10 <- glm(formula = as.formula(
  death6week ~ 
...
model10_results <- as.data.frame(summary(model10)$coefficients)
model10_results$name <- rownames(model10_results)

model10_results <- model10_results[ ,c('name', 'Estimate', 'Pr(>|z|)')]
colnames(model10_results) <- c('Variable', 'Estimate', 'PValue')
rownames(model10_results) <- seq(1:nrow(model10_results))
model10_results <- model10_results %>% 
  mutate(Estimate = round(Estimate, 3),
           PValue = round(PValue, 3))

model10_results$Variable <- c('Intercept', 'CPO', 'PAPi', 'API', 
                             'Albumin', 'Log Bilirubin+1', 
                             'eGFR', 'Sodium', 
                             'Log BNP+1', 'NT-proBNP',
                             'LVAD', 'Short MCS Ever', 
...
```{r, echo=F, warning=F, comment=NA, fig.align='center'}
model11 <- glm(formula = as.formula(
  death6week ~ 
...
model11_results <- as.data.frame(summary(model11)$coefficients)
model11_results$name <- rownames(model11_results)

model11_results <- model11_results[ ,c('name', 'Estimate', 'Pr(>|z|)')]
colnames(model11_results) <- c('Variable', 'Estimate', 'PValue')
rownames(model11_results) <- seq(1:nrow(model11_results))
model11_results <- model11_results %>% 
  mutate(Estimate = round(Estimate, 3),
           PValue = round(PValue, 3))

model11_results$Variable <- c('Intercept', 'CPO', 'PAPi', 'AST', 
                             'Albumin', 'Log Bilirubin+1', 
                             'eGFR', 'Sodium',                              
                             'Log BNP+1', 'NT-proBNP',
                             'LVAD', 'Short MCS Ever', 
...
model_comparison <- data.frame(
  'Model' = c('1 (French)', 
              '2 (New French)',
              '3 (US-CRS with PCWP)', 
              '4 (US-CRS with CPO and API)', 
              '5 (US-CRS with CPO Only)',
              '6 (US-CRS with API Only)',
              '7 (US-CRS with CPO and PAPi)',
              '8 (US-CRS with PAPi Only)',
              '9 (US-CRS with PAPi, API, AST)',
              '10 (US-CRS with PAPi and API)',
              '11 (US-CRS with PAPi and AST)'),
  ...
  'BIC' = c(BIC(model1), BIC(model2), BIC(model3), BIC(model4), BIC(model5), 
            BIC(model6), BIC(model7), BIC(model8), BIC(model9), BIC(model10),
            BIC(model11)))
...
### Final US-CRS model (no interaction term)
...
model_final_results$Variable <- c('Intercept', 'CPO', 'PAPi', 'Albumin', 'Log Bilirubin+1',
                             'eGFR', 'Sodium', 
                             'Log BNP+1', 'NT-proBNP',
                             'LVAD', 'Short MCS Ever')
...
df_coefplot$french_est[df_coefplot$var == 'short_MCS_ever'] <- summary(model2)$coefficients['short_MCS_French_new', 'Estimate']
df_coefplot$french_est[df_coefplot$var == 'log(bilirubin + 1)'] <- 
  summary(model2)$coefficients['log(bilirubin + 1)', 'Estimate']
df_coefplot$french_est[df_coefplot$var == 'eGFR'] <- summary(model2)$coefficients['eGFR', 'Estimate']
df_coefplot$french_est[df_coefplot$var == 'log(BNP + 1)'] <- summary(model2)$coefficients['log(BNP + 1)', 'Estimate']


df_coefplot$french_stderr[df_coefplot$var == 'short_MCS_ever'] <- 
  summary(model2)$coefficients['short_MCS_French_new', 'Std. Error']
df_coefplot$french_stderr[df_coefplot$var == 'log(bilirubin + 1)'] <- 
  summary(model2)$coefficients['log(bilirubin + 1)', 'Std. Error']
df_coefplot$french_stderr[df_coefplot$var == 'eGFR'] <- summary(model2)$coefficients['eGFR', 'Std. Error']
df_coefplot$french_stderr[df_coefplot$var == 'log(BNP + 1)'] <- summary(model2)$coefficients['log(BNP + 1)', 'Std. Error']
...
'eGFR', 'Sodium', 'Log BNP', 'NT-proBNP', 'Durable LVAD', 'Short-Term MCS')
...
df_coefplot <- df_coefplot %>%
  arrange(sapply(var, 
                 function(x) 
                   which(x == c('Short-Term MCS', 'Log Bilirubin', 
                                'eGFR', 'Log BNP', 'NT-proBNP', 
...
df_coefplot %>%
  ...
  geom_vline(xintercept = 0, lty = 2, color = '#880d1e') +
  ...
  scale_y_discrete(labels=c('Durable LVAD', expression('Sodium'^'b'), 'Albumin', 
                            expression('PAPi'^'c'), 'CPO', 'NT-proBNP', 'Log BNP', 
...
model2_predict <- as.numeric(
  predict(model2, newdata = df_intervals_test, type = 'response'))
model2_indices <- which(!is.na(model2_predict))
model2_predict <- na.omit(model2_predict)
...
df_intervals_test_model2 <- df_intervals_test[(as.numeric(rownames(df_intervals_test)) %in% model2_indices), ]
df_intervals_test_model_final <- df_intervals_test[(as.numeric(rownames(df_intervals_test)) %in% model_final_indices), ]
```
...
### ROC curve for models with no memory variables (eFigure 5)
...
pROC::roc(df_intervals_test_model1$death6week ~ model2_predict, direction='<', 
          plot=T, print.auc=T, ci=T, ci.type = 'no',
...
### ROC curve for models with all memory variables (eFigure 5)
...
legend('bottomright', cex = 1.2, xpd = T,
      legend = c('6-Status', 'New French CRS', 'US CRS'),
       col = c('#880d1e', '#a37c40', '#3f88c5'), lwd=3)
```
...
# 7. Variable importance plots from gradient-boosted models
...
# 8. Create calibration plot in test dataset (Figure 3)
...
# 9. Create facet plot of 50 point score at initial listing (Figure 4)
