...
<!-- 1. Set up dataset with generated cardiac variables (Line 42) -->
<!-- 2. Set up matrices for predictions (Line 155) -->
<!-- 3. Run selected logistic regression models and create coefficient plots (Line 608) -->
<!-- 4. Run sensitivity logistic regression models (Line 1693) -->
<!-- 5. Obtain machine learning model results (Line 1771) -->
<!-- 6. Make predictions on test dataset (Line 1833) -->
<!-- 7. Create AUC curves (Line 1894) -->
<!-- 8. Variable importance plots from gradient-boosted models (Line 2168) -->
<!-- 9. Create calibration plot in test dataset (Line 2229) -->
<!-- 10. Create facet plot of 50 point score at initial listing (Line 2338) -->
<!-- 11. Examine gender and race differences in prediction (Line 2368) -->
<!-- 12. Check for informative censoring (Line 2721) -->
<!-- 13. Examine a model with center random effects (Line 2754) -->
<!-- 14. Examine sensitivity and specificity at certain cut points (Line 2775) -->
<!-- 15. Examine model performance based on temporal split (Line 2821) -->
<!-- 16. Coefficient plot for 2 week outcome (Line 2889) -->
...
pacman::p_load(tidyverse, splines, knitr, kableExtra, glmnet, caret, gtsummary, 
               pROC, xgboost, survival, DescTools, cowplot, lme4,  
               lmtest, sandwich, rsample)
...
interval_dummies_train <- interval_dummies_train %>% select(-c(diagnosisDilated_CM))
...
mat_test <- data.frame(cbind(
  ...
  df_intervals[df_intervals$train_test == 'Test', ]$durable_LVAD, 
...
colnames(DMat_test) <- c('(Intercept)', 'Interval_1', 'Interval_2', 'Interval_3', 'Interval_4',
                         ...
                         'Durable_LVAD1', 'MCSD_Complication1', 'SystolicBP', 'DiastolicBP', 'PASP', 'PADP',
...
model_french_old_results <- as.data.frame(lmtest::coeftest(model_french_old, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_french_new_results <- as.data.frame(lmtest::coeftest(model_french_new, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_PCWP_results <- as.data.frame(lmtest::coeftest(model_PCWP, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_PCWP_results$Variable <- c('Intercept', 'PCWP', 'Albumin', 'Log Bilirubin+1', 
                             'eGFR', 'Sodium', 'Durable LVAD', 'Short MCS Ever', 
...
model_CPO_API_results <- as.data.frame(lmtest::coeftest(model_CPO_API, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_CPO_API_results$Variable <- c('Intercept', 'CPO', 'API', 'Albumin', 
                                    ...
                                    'Sodium', 'Durable LVAD', 'Short MCS Ever', 
                                    'Log BNP+1, Regular BNP',
                                    'Log BNP+1, NT-proBNP')
...
model_CPO_results <- as.data.frame(lmtest::coeftest(model_CPO, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_API_results <- as.data.frame(lmtest::coeftest(model_API, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_PAPI_CPO_results <- as.data.frame(lmtest::coeftest(model_PAPI_CPO, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_PAPI_results <- as.data.frame(lmtest::coeftest(model_PAPI, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_CPO_PAPI_AST_API_results <- as.data.frame(lmtest::coeftest(model_CPO_PAPI_AST_API, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_CPO_PAPI_API_results <- as.data.frame(lmtest::coeftest(model_CPO_PAPI_API, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_CPO_PAPI_AST_results <- as.data.frame(lmtest::coeftest(model_CPO_PAPI_AST, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_no_hemo_results <- as.data.frame(lmtest::coeftest(model_no_hemo, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_final_results <- as.data.frame(lmtest::coeftest(model_final, vcov=vcovCL, cluster=~PX_ID)[,])
...
model_final_hemo_results <- as.data.frame(lmtest::coeftest(model_final_hemo, vcov=vcovCL, cluster=~PX_ID)[,])
...
df_coefplot$french_stderr[df_coefplot$var == 'short_MCS_ever'] <- 
  as.data.frame(lmtest::coeftest(model_french_new, vcov=vcovCL, cluster=~PX_ID)[,])['short_MCS_French_new','Std. Error']
df_coefplot$french_stderr[df_coefplot$var == 'log(bilirubin + 1)'] <- 
  as.data.frame(lmtest::coeftest(model_french_new, vcov=vcovCL, cluster=~PX_ID)[,])['log(bilirubin + 1)','Std. Error']
df_coefplot$french_stderr[df_coefplot$var == 'eGFR'] <- 
  as.data.frame(lmtest::coeftest(model_french_new, vcov=vcovCL, cluster=~PX_ID)[,])['eGFR','Std. Error']

df_coefplot$french_stderr[df_coefplot$var == 'log(BNP + 1):BNP_NT_Pro0'] <- 
  as.data.frame(lmtest::coeftest(model_french_new, vcov=vcovCL, cluster=~PX_ID)[,])['log(BNP + 1):BNP_NT_Pro0','Std. Error']
df_coefplot$french_stderr[df_coefplot$var == 'log(BNP + 1):BNP_NT_Pro1'] <- 
  as.data.frame(lmtest::coeftest(model_french_new, vcov=vcovCL, cluster=~PX_ID)[,])['log(BNP + 1):BNP_NT_Pro1','Std. Error']
...
df_coefplot %>%
  ...
  xlab('Log-Odds Effect for 6-Week Waitlist Mortality') + xlim(-1.75, 1.75) +
...
```{r, warning=F, echo=F, message=F, comment=NA, fig.align='center', fig.height=8, fig.width=8}
df_coefplot <- as.data.frame(
  lmtest::coeftest(model_final_hemo, vcov=vcovCL, cluster=~PX_ID)[,])[ ,c('Estimate', 'Std. Error')]
df_coefplot$var <- rownames(df_coefplot)
rownames(df_coefplot) <- NULL
colnames(df_coefplot) <- c('us_est', 'us_stderr', 'var')
...
df_coefplot %>%
  ...
  geom_errorbarh(aes(xmin = lower, xmax = upper), position = position_dodge(width = 0.7)) +
  geom_vline(xintercept = 0, lty = 2, color = '#880d1e', linewidth = 0.05) +
  xlab('Log-Odds Effect for 6-Week Waitlist Mortality') + xlim(-1.5, 1.5) +
  scale_y_discrete(labels=c('Durable LVAD', expression('Sodium'^'b'), 'Albumin', 
...
model_final_BP <- glm(formula = as.formula(
  death6week ~ 
    ...
    eGFR + sodium + 
    (log(BNP + 1):BNP_NT_Pro) + 
    durable_LVAD + short_MCS_ever_with_BP_perc), 
  data = df_intervals[df_intervals$train_test == 'Train',], 
  family = binomial)

model_final_BP_results <- as.data.frame(
  lmtest::coeftest(model_final_BP, vcov=vcovCL, cluster=~PX_ID)[,])[ ,c('Estimate', 'Pr(>|z|)')]
...
model_hemo_BP_results <- as.data.frame(
  lmtest::coeftest(model_hemo_BP, vcov=vcovCL, cluster=~PX_ID)[,])[ ,c('Estimate', 'Pr(>|z|)')]
...
model_elastic_all_results <- data.frame(
  'Variable' = rownames(
    coef(model_elastic_all$finalModel, model_elastic_all$bestTune$lambda, model_elastic_all$bestTune$alpha, exact=F)),
  
  'Estimate' = as.numeric(coef(model_elastic_all$finalModel, model_elastic_all$bestTune$lambda,
                               model_elastic_all$bestTune$alpha, exact=F)))
...
### ROC curve for subset of models at first interval (Figure 3)

```{r, warning=F, echo=F, message=F, comment=NA, fig.align='center', fig.width=7, fig.height=6}
df_intervals_first <- df_intervals_test %>% filter(interval == 1)

df_intervals_french_first <- df_intervals_test_model_french_new %>% 
  select(PX_ID, interval, death6week)
df_intervals_french_first <- cbind(df_intervals_french_first, model_french_new_predict)
colnames(df_intervals_french_first) <- c('PX_ID', 'interval', 'death6week', 'predict')
df_intervals_french_first <- df_intervals_french_first %>% filter(interval == 1)

df_intervals_us_first <- df_intervals_test_model_final %>% 
  select(PX_ID, interval, death6week)
df_intervals_us_first <- cbind(df_intervals_us_first, model_final_predict)
colnames(df_intervals_us_first) <- c('PX_ID', 'interval', 'death6week', 'predict')
df_intervals_us_first <- df_intervals_us_first %>% filter(interval == 1)


# dev.new(width = 8, height = 8, noRStudioGD = T)
par(xaxs='i')
pROC::roc(data=df_intervals_first, response=death6week, predictor=status_initial, direction='>',
          ...
          print.auc.cex = 1.2, 
          legacy.axes=T)
pROC::roc(df_intervals_french_first$death6week ~ df_intervals_french_first$predict, direction='<',
          plot=T, print.auc=T, ci=T, ci.type = 'no',
          print.auc.y=0.45, smooth=T, lwd=2.5, col='#a37c40', add=T, quiet=T, print.auc.cex = 1.2)
pROC::roc(df_intervals_us_first$death6week ~ df_intervals_us_first$predict, direction='<',
          plot=T, print.auc=T, ci=T, ci.type = 'no',
...
### ROC curve for models with all memory variables (eFigure 3)

```{r, warning=F, echo=F, message=F, comment=NA, fig.align='center', fig.height=8, fig.width=8, results='hide'}
df_intervals_french_old_first <- df_intervals_test_model_french_old %>% 
  select(PX_ID, interval, death6week)
df_intervals_french_old_first <- 
  cbind(df_intervals_french_old_first, model_french_old_predict)
colnames(df_intervals_french_old_first) <- c('PX_ID', 'interval', 'death6week', 'predict')
df_intervals_french_old_first <- 
  df_intervals_french_old_first %>% filter(interval == 1)

df_intervals_elastic_first <- df_intervals_test_mem %>% 
  select(PX_ID, interval, death6week)
df_intervals_elastic_first <- 
  cbind(df_intervals_elastic_first, elastic_all_predict)
colnames(df_intervals_elastic_first) <- c('PX_ID', 'interval', 'death6week', 'predict')
df_intervals_elastic_first <- 
  df_intervals_elastic_first %>% filter(interval == 1)


df_intervals_xg_first <- df_intervals_test %>% 
  select(PX_ID, interval, death6week)
df_intervals_xg_first <- 
  cbind(df_intervals_xg_first, xg_all_predict)
colnames(df_intervals_xg_first) <- c('PX_ID', 'interval', 'death6week', 'predict')
df_intervals_xg_first <- 
  df_intervals_xg_first %>% filter(interval == 1)
...
pROC::roc(df_intervals_elastic_first$death6week ~ df_intervals_elastic_first$predict, direction='<', 
          plot=T, print.auc=T, ci=T, ci.type = 'no',  
          print.auc.y=0.45, smooth=T, lwd=2.5, col='wheat3', add=T, quiet=T)
pROC::roc(df_intervals_xg_first$death6week ~ df_intervals_xg_first$predict, direction='<',
          plot=T, print.auc=T, ci=T, ci.type = 'no', 
          print.auc.y=0.40, smooth=T, lwd=2.5, col='#a37c40', add=T, quiet=T)
pROC::roc(df_intervals_french_old_first$death6week ~ df_intervals_french_old_first$predict, direction='<',
          plot=T, print.auc=T, ci=T, ci.type = 'no',
...
### Time-Dependent AUC Calculations

```{r, warning=F, echo=F, message=F, comment=NA, fig.align='center', results='hide'}
interval_max <- floor(365/14)
df_intervals_test$french_predict <- predict(model_french_new, df_intervals_test)
df_intervals_test$us_predict <- predict(model_final, df_intervals_test)


time_dependent_auc_six <- tibble(interval = seq(1,interval_max))
six_auc <- vector(mode = "numeric")
six_low <- vector(mode = "numeric")
six_up<- vector(mode = "numeric")

for (i in time_dependent_auc_six %>% pull(interval)) {
  roc_i <- df_intervals_test %>%
    filter(interval == i) %>%
  roc(death6week, status_initial)
...
time_dependent_auc_six <- time_dependent_auc_six %>% cbind(six_auc, six_low, six_up) 


time_dependent_auc_french <- tibble(interval = seq(1,interval_max))
french_auc <- vector(mode = "numeric")
french_low <- vector(mode = "numeric")
french_up<- vector(mode = "numeric")

for (i in time_dependent_auc_french %>% pull(interval)) {
  roc_i <- df_intervals_test %>%
    filter(interval == i) %>%
  roc(death6week, french_predict)
  
  roc_ci <- ci.auc(roc_i)
  
  french_auc[[i]] <- roc_ci[[2]]
  french_low[[i]] <- roc_ci[[1]]
  french_up[[i]] <- roc_ci[[3]]
}
time_dependent_auc_french<- time_dependent_auc_french %>% cbind(french_auc, french_low, french_up) 


time_dependent_auc_us <- tibble(interval = seq(1,interval_max))
us_auc <- vector(mode = "numeric")
us_low <- vector(mode = "numeric")
us_up<- vector(mode = "numeric")

for (i in time_dependent_auc_us %>% pull(interval)) {
  roc_i <- df_intervals_test %>%
    filter(interval == i) %>%
  roc(death6week, us_predict)
...
time_dependent_auc_us<- time_dependent_auc_us %>% cbind(us_auc, us_low, us_up)



# dev.new(width = 8, height = 8, noRStudioGD = T)
par(xaxs='i')
time_dependent_auc_six %>%
  left_join(time_dependent_auc_us) %>%
  left_join(time_dependent_auc_french) %>%
  pivot_longer(cols = -interval,
               names_to = c( "Score", ".value"),
               names_sep = "_") %>%
  mutate(Score = case_when(
    Score == "six" ~ "6-Status",
    Score == "us" ~ "US-CRS",
    Score == "french" ~ "French-CRS")) %>%
  ggplot(aes(x = interval*14, y = auc, ymin = low, ymax = up, color = Score)) +
  geom_point() + 
  geom_line() + 
  geom_errorbar(alpha = 0.3) + 
  geom_hline(yintercept = 0.5, color = 'gray') +
  scale_x_continuous(breaks = seq(0, 300, 100)) +
  scale_color_manual(values = c('#a37c40','#880d1e', '#3f88c5')) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
  ...
  labs(x = "Days on Waitlist", y = "AUC (6-Week Mortality)") 
```


### ROC curve for subset of models at specified intervals
```{r, warning=F, echo=F, message=F, comment=NA, fig.align='center', fig.width=7, fig.height=6}
df_intervals_18 <- df_intervals_test %>% filter(interval == 18)

df_intervals_french_18 <- df_intervals_test_model_french_new %>% 
  select(PX_ID, interval, death6week)
df_intervals_french_18 <- cbind(df_intervals_french_18, model_french_new_predict)
colnames(df_intervals_french_18) <- c('PX_ID', 'interval', 'death6week', 'predict')
df_intervals_french_18 <- df_intervals_french_18 %>% filter(interval == 18)

df_intervals_us_18 <- df_intervals_test_model_final %>% 
  select(PX_ID, interval, death6week)
df_intervals_us_18 <- cbind(df_intervals_us_18, model_final_predict)
colnames(df_intervals_us_18) <- c('PX_ID', 'interval', 'death6week', 'predict')
df_intervals_us_18 <- df_intervals_us_18 %>% filter(interval == 18)


# dev.new(width = 8, height = 8, noRStudioGD = T)
par(xaxs='i')
pROC::roc(data=df_intervals_18, response=death6week, predictor=status_initial, direction='>',
          plot=T, col='#880d1e', print.auc=T, ci=T, ci.type = 'no',
          lwd=3, quiet=T, print.auc.y=0.35,
          print.auc.cex = 1.2, 
          legacy.axes=T)
pROC::roc(df_intervals_french_18$death6week ~ df_intervals_french_18$predict, direction='<',
          plot=T, print.auc=T, ci=T, ci.type = 'no',
          print.auc.y=0.30, smooth=T, lwd=2.5, col='#a37c40', add=T, quiet=T, print.auc.cex = 1.2)
pROC::roc(df_intervals_us_18$death6week ~ df_intervals_us_18$predict, direction='<',
          plot=T, print.auc=T, ci=T, ci.type = 'no',
          print.auc.y=0.25, smooth=T, lwd=2.5, col='#3f88c5', add=T, quiet=T, print.auc.cex = 1.2)
legend('bottomright', xpd = T, 
      legend = c('6-Status', 'Current French-CRS', 'US-CRS'),
       col = c('#880d1e', '#a37c40', '#3f88c5'), lwd=2.5,  cex=1.1)
...
df_intervals_20 <- df_intervals_test %>% filter(interval == 20)

df_intervals_french_20 <- df_intervals_test_model_french_new %>% 
  select(PX_ID, interval, death6week)
df_intervals_french_20 <- cbind(df_intervals_french_20, model_french_new_predict)
colnames(df_intervals_french_20) <- c('PX_ID', 'interval', 'death6week', 'predict')
df_intervals_french_20 <- df_intervals_french_20 %>% filter(interval == 20)

df_intervals_us_20 <- df_intervals_test_model_final %>% 
  select(PX_ID, interval, death6week)
df_intervals_us_20 <- cbind(df_intervals_us_20, model_final_predict)
colnames(df_intervals_us_20) <- c('PX_ID', 'interval', 'death6week', 'predict')
df_intervals_us_20 <- df_intervals_us_20 %>% filter(interval == 20)


# dev.new(width = 8, height = 8, noRStudioGD = T)
par(xaxs='i')
pROC::roc(data=df_intervals_20, response=death6week, predictor=status_initial, direction='>',
          plot=T, col='#880d1e', print.auc=T, ci=T, ci.type = 'no',
          lwd=3, quiet=T, print.auc.y=0.35,
          print.auc.cex = 1.2, 
          legacy.axes=T)
pROC::roc(df_intervals_french_20$death6week ~ df_intervals_french_20$predict, direction='<',
          plot=T, print.auc=T, ci=T, ci.type = 'no',
          print.auc.y=0.30, smooth=T, lwd=2.5, col='#a37c40', add=T, quiet=T, print.auc.cex = 1.2)
pROC::roc(df_intervals_us_20$death6week ~ df_intervals_us_20$predict, direction='<',
          plot=T, print.auc=T, ci=T, ci.type = 'no',
          print.auc.y=0.25, smooth=T, lwd=2.5, col='#3f88c5', add=T, quiet=T, print.auc.cex = 1.2)
legend('bottomright', xpd = T, 
      legend = c('6-Status', 'Current French-CRS', 'US-CRS'),
...
importance1 <- importance1[importance1$Gain > 0.04, ]
...
importance2 <- importance2[importance2$Gain > 0.04, ]
...
# 9. Create calibration plot in test dataset (eFigure 3)
...
df_intervals_calib <- df_intervals
df_intervals_calib$death6week <- as.numeric(as.character(df_intervals_calib$death6week))
df_intervals_test_calib <- subset(df_intervals_calib, df_intervals_calib$train_test == 'Test')
df_intervals_test_calib <- df_intervals_test_calib %>% select(PX_ID, death6week, 
                                                              uscrs_predict, uscrs_score)
df_intervals_test_calib <- subset(df_intervals_test_calib, !is.na(uscrs_score))


df_calib_nest <- df_intervals_test_calib %>% nest(-PX_ID)
set.seed(200)
n_iterations <- 2000
bs <- rsample::bootstraps(df_calib_nest, times = n_iterations)

se_cut1 <- vector(length = n_iterations)
se_cut2 <- vector(length = n_iterations)
se_cut3 <- vector(length = n_iterations)
se_cut4 <- vector(length = n_iterations)
se_cut5 <- vector(length = n_iterations)
se_cut6 <- vector(length = n_iterations)
se_cut7 <- vector(length = n_iterations)
se_cut8 <- vector(length = n_iterations)
se_cut9 <- vector(length = n_iterations)
se_cut10 <- vector(length = n_iterations)
...
for (i in 1:n_iterations) {
  ...
  se_cut1[i] <- result_df$se[result_df$decile_predictions == '1']
  se_cut2[i] <- result_df$se[result_df$decile_predictions == '2']
  se_cut3[i] <- result_df$se[result_df$decile_predictions == '3']
  se_cut4[i] <- result_df$se[result_df$decile_predictions == '4']
  se_cut5[i] <- result_df$se[result_df$decile_predictions == '5']
  se_cut6[i] <- result_df$se[result_df$decile_predictions == '6']
  se_cut7[i] <- result_df$se[result_df$decile_predictions == '7']
  se_cut8[i] <- result_df$se[result_df$decile_predictions == '8']
  se_cut9[i] <- result_df$se[result_df$decile_predictions == '9']
  se_cut10[i] <- result_df$se[result_df$decile_predictions == '10']
}



df_intervals_test_calib <- df_intervals_test_calib %>%
  mutate(decile_predictions = cut_number(uscrs_predict, 10, labels= FALSE)) %>%
  group_by(decile_predictions) %>%
  summarise(predicted = mean(uscrs_predict),
            observed = mean(death6week),
  ...
  mutate(se = case_when(decile_predictions == '1' ~ mean(se_cut1),
                        decile_predictions == '2' ~ mean(se_cut2),
                        decile_predictions == '3' ~ mean(se_cut3),
                        decile_predictions == '4' ~ mean(se_cut4),
                        decile_predictions == '5' ~ mean(se_cut5),
                        decile_predictions == '6' ~ mean(se_cut6),
                        decile_predictions == '7' ~ mean(se_cut7),
                        decile_predictions == '8' ~ mean(se_cut8),
                        decile_predictions == '9' ~ mean(se_cut9),
                        decile_predictions == '10' ~ mean(se_cut10))) %>%
  mutate(upper_ci = observed + 1.96*se,
...
lmtest::coeftest(lm(observed ~ predicted, data = df_intervals_test_calib), vcov=vcovCL)
```
...
# 10. Create facet plot of 50 point score at initial listing (Figure 4)
...
df_gender_race <- df_intervals %>% select(PX_ID, interval, uscrs_predict, 
...
df_gender_race %>% ungroup() %>% filter(interval==1) %>%
...
###############################################################
df_gender_race_nest <- df_gender_race %>% filter(female == 'Female') %>% 
  nest(-PX_ID)
bs <- rsample::bootstraps(df_gender_race_nest, times = n_iterations)
se_cut1 <- vector(length = n_iterations)
se_cut2 <- vector(length = n_iterations)
se_cut3 <- vector(length = n_iterations)
se_cut4 <- vector(length = n_iterations)
...
###############################################################
df_gender_race_nest <- df_gender_race %>% filter(female == 'Male') %>% 
  nest(-PX_ID)
bs <- rsample::bootstraps(df_gender_race_nest, times = n_iterations)
se_cut1 <- vector(length = n_iterations)
se_cut2 <- vector(length = n_iterations)
...
###############################################################  
df_gender_race_nest <- df_gender_race %>% filter(race == 'Not White') %>% 
  nest(-PX_ID)
bs <- rsample::bootstraps(df_gender_race_nest, times = n_iterations)
se_cut1 <- vector(length = n_iterations)
se_cut2 <- vector(length = n_iterations)
...
###############################################################  
df_gender_race_nest <- df_gender_race %>% filter(race == 'White') %>% 
  nest(-PX_ID)
bs <- rsample::bootstraps(df_gender_race_nest, times = n_iterations)
se_cut1 <- vector(length = n_iterations)
se_cut2 <- vector(length = n_iterations)
...
```{r, echo=F, warning=F, message=F, comment=NA}
# Change missing center ID to 0 (unique identifier, prevents missing errors)
df_intervals$CenterId[is.na(df_intervals$CenterId)] <- '0'


glmer(formula = as.formula(
  death6week ~ 
    albumin + log(bilirubin + 1) + 
    eGFR + sodium + 
    (log(BNP + 1):BNP_NT_Pro) + 
    durable_LVAD + short_MCS_ever + (1 | CenterId)), 
  data = df_intervals[df_intervals$train_test == 'Train',],
  family = binomial)
```
...
```{r, echo=F, warning=F, message=F, comment=NA}
sixstatus_auc <- roc(data=df_intervals_first,
                     response=death6week,
                     predictor=status_initial, direction='>', quiet=T)
french_auc <- roc(df_intervals_french_first$death6week ~ df_intervals_french_first$predict, direction='<')
us_auc <- roc(df_intervals_us_first$death6week ~ df_intervals_us_first$predict, direction='<')


pROC::ci.coords(sixstatus_auc, x=1, input='threshold')
pROC::ci.coords(sixstatus_auc, x=2, input='threshold')
pROC::ci.coords(sixstatus_auc, x=3, input='threshold')
pROC::ci.coords(sixstatus_auc, x=4, input='threshold')
pROC::ci.coords(sixstatus_auc, x=5, input='threshold')

pROC::ci.coords(french_auc, x=0.214, input='sensitivity')
pROC::ci.coords(french_auc, x=0.622, input='sensitivity')
pROC::ci.coords(french_auc, x=0.735, input='sensitivity')
pROC::ci.coords(french_auc, x=0.878, input='sensitivity')
pROC::ci.coords(french_auc, x=0.908, input='sensitivity')

pROC::ci.coords(french_auc, x=0.949, input='specificity')
pROC::ci.coords(french_auc, x=0.680, input='specificity')
pROC::ci.coords(french_auc, x=0.562, input='specificity')
pROC::ci.coords(french_auc, x=0.180, input='specificity')
pROC::ci.coords(french_auc, x=0.152, input='specificity')


pROC::ci.coords(us_auc, x=0.214, input='sensitivity')
pROC::ci.coords(us_auc, x=0.622, input='sensitivity')
pROC::ci.coords(us_auc, x=0.735, input='sensitivity')
pROC::ci.coords(us_auc, x=0.878, input='sensitivity')
pROC::ci.coords(us_auc, x=0.908, input='sensitivity')

pROC::ci.coords(us_auc, x=0.949, input='specificity')
pROC::ci.coords(us_auc, x=0.680, input='specificity')
pROC::ci.coords(us_auc, x=0.562, input='specificity')
pROC::ci.coords(us_auc, x=0.180, input='specificity')
pROC::ci.coords(us_auc, x=0.152, input='specificity')
```
...
# 16. Coefficient plot for 2 week outcome

## Create 2 week death variable
```{r, warning=F, message=F, comment=NA}
## General death flag (non-last group)
death_ids <- unique(df_intervals$PX_ID[df_intervals$death6week == '1'])
...
df_intervals$death2week <- 0
df_intervals$deathdifference <- (as.numeric(df_intervals$death_time) - 
                                   as.numeric(df_intervals$interval_stop))
df_intervals$death2week[df_intervals$death_flag == '1' & df_intervals$deathdifference <= 14] <- 1

## If transplanted, no death within 2 weeks in any interval
df_intervals$death2week[df_intervals$transplant == '1'] <- '0'
df_intervals$death2week <- as.numeric(df_intervals$death2week)
```


## Run models
```{r, warning=F, message=F, comment=NA}
model_final_2week <- glm(formula = as.formula(
  death2week ~ 
    albumin + log(bilirubin + 1) + 
    eGFR + sodium + 
    (log(BNP + 1):BNP_NT_Pro) + 
...
```{r, echo=F, warning=F, message=F, comment=NA}
df_coefplot <- as.data.frame(
  lmtest::coeftest(model_final_2week, vcov=vcovCL, cluster=~PX_ID)[,])[ ,c('Estimate', 'Std. Error')]
df_coefplot$var <- rownames(df_coefplot)
rownames(df_coefplot) <- NULL
colnames(df_coefplot) <- c('two_est', 'two_stderr', 'var')

df_coefplot$six_est <- NA
df_coefplot$six_stderr <- NA

df_coefplot$six_est[df_coefplot$var == 'short_MCS_ever'] <- 
  summary(model_final)$coefficients['short_MCS_ever', 'Estimate']
df_coefplot$six_est[df_coefplot$var == 'log(bilirubin + 1)'] <- 
  summary(model_final)$coefficients['log(bilirubin + 1)', 'Estimate']
df_coefplot$six_est[df_coefplot$var == 'eGFR'] <- 
  summary(model_final)$coefficients['eGFR', 'Estimate']
df_coefplot$six_est[df_coefplot$var == 'sodium'] <- 
  summary(model_final)$coefficients['sodium', 'Estimate']
df_coefplot$six_est[df_coefplot$var == 'albumin'] <- 
  summary(model_final)$coefficients['albumin', 'Estimate']
df_coefplot$six_est[df_coefplot$var == 'durable_LVAD1'] <- 
  summary(model_final)$coefficients['durable_LVAD1', 'Estimate']
df_coefplot$six_est[df_coefplot$var == 'log(BNP + 1):BNP_NT_Pro0'] <- 
  summary(model_final)$coefficients['log(BNP + 1):BNP_NT_Pro0', 'Estimate']
df_coefplot$six_est[df_coefplot$var == 'log(BNP + 1):BNP_NT_Pro1'] <- 
  summary(model_final)$coefficients['log(BNP + 1):BNP_NT_Pro1', 'Estimate']



df_coefplot$six_stderr[df_coefplot$var == 'short_MCS_ever'] <- 
  df_coefplot2$us_stderr[df_coefplot2$var == 'short_MCS_ever']
df_coefplot$six_stderr[df_coefplot$var == 'log(bilirubin + 1)'] <- 
  df_coefplot2$us_stderr[df_coefplot2$var == 'log(bilirubin + 1)']
df_coefplot$six_stderr[df_coefplot$var == 'eGFR'] <- 
  df_coefplot2$us_stderr[df_coefplot2$var == 'eGFR']
df_coefplot$six_stderr[df_coefplot$var == 'sodium'] <- 
  df_coefplot2$us_stderr[df_coefplot2$var == 'sodium']
df_coefplot$six_stderr[df_coefplot$var == 'albumin'] <- 
  df_coefplot2$us_stderr[df_coefplot2$var == 'albumin']
df_coefplot$six_stderr[df_coefplot$var == 'durable_LVAD1'] <- 
   df_coefplot2$us_stderr[df_coefplot2$var == 'durable_LVAD1']
df_coefplot$six_stderr[df_coefplot$var == 'log(BNP + 1):BNP_NT_Pro0'] <- 
  df_coefplot2$us_stderr[df_coefplot2$var == 'log(BNP + 1):BNP_NT_Pro0']
df_coefplot$six_stderr[df_coefplot$var == 'log(BNP + 1):BNP_NT_Pro1'] <- 
  df_coefplot2$us_stderr[df_coefplot2$var == 'log(BNP + 1):BNP_NT_Pro1']


df_coefplot <- df_coefplot[ ,c('var', 'six_est', 'six_stderr', 'two_est', 'two_stderr')]
df_coefplot <- subset(df_coefplot, df_coefplot$var != '(Intercept)')
...
### Scale certain estimates
df_coefplot$six_est[df_coefplot$var == 'eGFR'] <- df_coefplot$six_est[df_coefplot$var == 'eGFR'] * 10
df_coefplot$six_est[df_coefplot$var == 'Sodium'] <- df_coefplot$six_est[df_coefplot$var == 'Sodium'] * 10

df_coefplot$six_stderr[df_coefplot$var == 'eGFR'] <- df_coefplot$six_stderr[df_coefplot$var == 'eGFR'] * 10
df_coefplot$six_stderr[df_coefplot$var == 'Sodium'] <- df_coefplot$six_stderr[df_coefplot$var == 'Sodium'] * 10


df_coefplot$two_est[df_coefplot$var == 'eGFR'] <- df_coefplot$two_est[df_coefplot$var == 'eGFR'] * 10
df_coefplot$two_est[df_coefplot$var == 'Sodium'] <- df_coefplot$two_est[df_coefplot$var == 'Sodium'] * 10

df_coefplot$two_stderr[df_coefplot$var == 'eGFR'] <- df_coefplot$two_stderr[df_coefplot$var == 'eGFR'] * 10
df_coefplot$two_stderr[df_coefplot$var == 'Sodium'] <- df_coefplot$two_stderr[df_coefplot$var == 'Sodium'] * 10



df_coefplot <- df_coefplot %>%
  mutate(
    six_lower = six_est - (1.96*six_stderr),
    six_upper = six_est + (1.96*six_stderr),
    two_lower = two_est - (1.96*two_stderr),
    two_upper = two_est + (1.96*two_stderr)) %>%
  select(-c(six_stderr, two_stderr))


df_coefplot <- df_coefplot %>%
  pivot_longer(!var, names_to = 'stat', values_to = 'count')
df_coefplot$model <- ifelse(grepl('^two', df_coefplot$stat), 'Two', 'Six')
df_coefplot$stat <- sub('.*_', '', df_coefplot$stat)
...
df_coefplot %>%
  ...
  geom_errorbarh(aes(xmin = lower, xmax = upper), position = position_dodge(width = 0.7)) +
  geom_vline(xintercept = 0, lty = 2, color = '#880d1e', linewidth = 0.05) +
  xlab('Log-Odds Effect for 6-Week Waitlist Mortality') + xlim(-2, 2) +
  scale_y_discrete(labels=c('Durable LVAD', expression('Sodium'^'b'), 'Albumin', 
                            'Log BNP, NT-proBNP', 
                            'Log BNP, Regular BNP', 
                            expression('eGFR'^'b'), 'Log Bilirubin', 
...
```{r, echo=F, warning=F, message=F, comment=NA}
library(risksetROC)
model_french_2week <- glm(formula = as.formula(
  death2week ~ 
    short_MCS_French_new + log(bilirubin + 1) + eGFR + log(BNP + 1):BNP_NT_Pro), 
  data = df_intervals[df_intervals$train_test == 'Train',], 
  family = binomial)

df_intervals$us_2week_predict <- predict(model_final_2week, df_intervals)
df_intervals$french_2week_predict <- predict(model_french_2week, df_intervals)


df_intervals_new <- subset(df_intervals, !is.na(df_intervals$us_2week_predict) &
                         !is.na(df_intervals$french_2week_predict) &
                         df_intervals$train_test == 'Test')
risksetAUC(
  Stime= df_intervals_new$interval_stop,
  entry = df_intervals_new$interval_start,
  status = df_intervals_new$death2week,
  marker = df_intervals_new$french_2week_predict,
  method = 'Cox',
  tmax = 365*2,
  plot = F)
...
risksetAUC(
  ...
  entry = df_intervals_new$interval_start,
  status = df_intervals_new$death2week,
  marker = df_intervals_new$us_2week_predict,
  method = 'Cox',
  tmax = 365*2,
  plot = F)
```
