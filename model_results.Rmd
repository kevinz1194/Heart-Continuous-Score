...
<!-- 1. Set up dataset with generated cardiac variables (Line 35) -->
<!-- 2. Set up matrices for predictions (Line 143) -->
<!-- 3. Run selected logistic regression models and create coefficient plots (Line 599) -->
<!-- 4. Run sensitivity logistic regression models (Line 1293) -->
<!-- 5. Obtain machine learning model results (Line 1334) -->
<!-- 6. Make predictions on test dataset (Line 1396) -->
<!-- 7. Create AUC curves (Line 1450) -->
<!-- 8. Variable importance plots from gradient-boosted models (Line 1506) -->
<!-- 9. Create calibration plot in test dataset (Line 1567) -->
<!-- 10. Create facet plot of 50 point score at initial listing (Line 1615) -->
...
df_intervals <- df_intervals %>%
  ...
  mutate(short_MCS_ever = 
         ...
         short_MCS_ever_with_BP = case_when(
           IABP_ever == '1' ~ 1,
           TRUE ~ short_MCS_ever))
...
model_french_old_results$Variable <- c('Intercept', 'Short Term MCS', 'Log Bilirubin+1', 'eGFR', 'Log BNP+1')
...
model_french_new_results$Variable <- c('Intercept', 'Short Term MCS (New)', 'Log Bilirubin+1', 'eGFR', 'Log BNP+1')
...
model_comparison <- data.frame(
  ...
  'AIC' = c(AIC(model_french_old), 
    ...
    'BIC' = c(BIC(model_french_old), 
            BIC(model_french_new), 
            BIC(model_PCWP), 
            BIC(model_CPO_API), 
            BIC(model_CPO), 
            BIC(model_API), 
            BIC(model_PAPI_CPO), 
            BIC(model_PAPI), 
            BIC(model_CPO_PAPI_AST_API), 
            BIC(model_CPO_PAPI_API),
            BIC(model_CPO_PAPI_AST)))
...
model_final_results$Variable <- c('Intercept', 'CPO', 
                                  'PAPi', 'Albumin', 'Log Bilirubin+1',
                                  'eGFR', 'Sodium', 
                                  'LVAD', 'Short MCS Ever',
                                  'Log BNP+1, Regular BNP',
...
### Assumes the French coefficient for BNP is for non nT-proBNP
df_coefplot$french_est[df_coefplot$var == 'log(BNP + 1):BNP_NT_Pro0'] <- 
  summary(model_french_new)$coefficients['log(BNP + 1)', 'Estimate']
...
df_coefplot$french_stderr[df_coefplot$var == 'log(BNP + 1):BNP_NT_Pro0'] <- 
  summary(model_french_new)$coefficients['log(BNP + 1)', 'Std. Error']
...
### US-CRS with balloon pumps in short-term MCS
