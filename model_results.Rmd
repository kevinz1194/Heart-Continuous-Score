...
for (i in time_dependent_auc_six %>% pull(interval)) {
  roc_i <- df_intervals_test %>%
    filter(interval == i) %>%
  roc(death6week, status_initial, direction='>')
...
pROC::ci.coords(french_auc, x=0.576, input='specificity')
pROC::ci.coords(french_auc, x=0.192, input='specificity')
...
pROC::ci.coords(us_auc, x=0.576, input='specificity')
pROC::ci.coords(us_auc, x=0.192, input='specificity')
...
df_coefplot$six_est[df_coefplot$var == 'LVAD1'] <- 
  summary(model_final)$coefficients['LVAD1', 'Estimate']
...
df_coefplot$six_stderr[df_coefplot$var == 'LVAD1'] <- 
   df_coefplot2$us_stderr[df_coefplot2$var == 'LVAD1']
