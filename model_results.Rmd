...
<!-- 2. Set up matrices for predictions (Line 137) -->
<!-- 3. Run selected logistic regression models and create coefficient plots (Line 590) -->
<!-- 4. Obtain machine learning model results (Line 1114) -->
<!-- 5. Make predictions on test dataset (Line 1172) -->
<!-- 6. Create AUC curves (Line 1213) -->
<!-- 7. Variable importance plots from gradient-boosted models (Line 1284) -->
<!-- 8. Create calibration plot in test dataset (Line 1345) -->
<!-- 9. Create facet plot of 50 point score at initial listing (Line 1393) -->
...
### Selected Model #8: US candidate risk score with PAPi, AST, and API

```{r, echo=F, warning=F, comment=NA, fig.align='center'}
model8 <- glm(formula = as.formula(
  death6week ~ 
    cpo + papi + api + AST + albumin + log(bilirubin + 1) + eGFR + sodium + log(BNP + 1) + LVAD + short_MCS_ever), 
  data = df_intervals[df_intervals$train_test == 'Train',], 
  family = binomial)
...
model8_results$Variable <- c('Intercept', 'CPO', 'PAPi', 'API', 'AST',
                             'Albumin', 'Log Bilirubin+1', 'eGFR', 'Sodium', 'Log BNP+1',
                             'LVAD', 'Short MCS Ever')
...
### Selected Model #9: US candidate risk score with PAPi and API

```{r, echo=F, warning=F, comment=NA, fig.align='center'}
model9 <- glm(formula = as.formula(
  death6week ~ 
    cpo + papi + api + albumin + log(bilirubin + 1) + eGFR + sodium + log(BNP + 1) + LVAD + short_MCS_ever), 
  data = df_intervals[df_intervals$train_test == 'Train',], 
  family = binomial)
...
model9_results$Variable <- c('Intercept', 'CPO', 'PAPi', 'API', 
                             'Albumin', 'Log Bilirubin+1', 'eGFR', 'Sodium', 'Log BNP+1',
                             'LVAD', 'Short MCS Ever')
...
### Selected Model #10: US candidate risk score with PAPi and AST

```{r, echo=F, warning=F, comment=NA, fig.align='center'}
model10 <- glm(formula = as.formula(
  death6week ~ 
    cpo + papi + AST + albumin + log(bilirubin + 1) + eGFR + sodium + log(BNP + 1) + LVAD + short_MCS_ever), 
  data = df_intervals[df_intervals$train_test == 'Train',], 
  family = binomial)
...
model10_results$Variable <- c('Intercept', 'CPO', 'PAPi', 'AST', 
                             'Albumin', 'Log Bilirubin+1', 'eGFR', 'Sodium', 'Log BNP+1',
                             'LVAD', 'Short MCS Ever')
...
model_comparison <- data.frame(
  'Model' = c('1 (French)', 
              ...
              '8 (US-CRS with PAPi, API, AST)',
              '9 (US-CRS with PAPi and API)',
              '10 (US-CRS with PAPi and AST)'),
...
df_coefplot %>%
  ...
  scale_y_discrete(labels=c('Durable LVAD', expression('Sodium'^'b'), 'Albumin', expression('PAPi'^'c'), 
                             'CPO', 'Log BNP', expression('eGFR'^'b'), 'Log Bilirubin', 
...
pROC::roc(data=df_intervals_test, response=death6week, predictor=status_initial, direction='>', 
          plot=T, col='black', print.auc=T, lwd=2.5, quiet=T)
pROC::roc(df_intervals_test_nomem$death6week ~ elastic_nomem_predict, direction='<', 
          plot=T, print.auc=T, print.auc.y=0.45, smooth=T, lwd=2.5, col='#fdbe85', add=T, quiet=T)
pROC::roc(df_intervals_test$death6week ~ xg_nomem_predict, direction='<', 
          plot=T, print.auc=T, print.auc.y=0.40, smooth=T, lwd=2.5, col='#fd8d3c', add=T, quiet=T)
pROC::roc(df_intervals_test_model1$death6week ~ model1_predict, direction='<', 
          plot=T, print.auc=T, print.auc.y=0.35, smooth=T, lwd=2.5, col='#e6550d', add=T, quiet=T)
pROC::roc(df_intervals_test_model6$death6week ~ model6_predict, direction='<', 
          plot=T, print.auc=T, print.auc.y=0.30, smooth=T, lwd=2.5, col='#a63603', add=T, quiet=T)
legend('bottomright', cex = 0.9, xpd = T,
      legend = c('6-Status', 
       ...
       col = c('black', '#fdbe85', '#fd8d3c', '#e6550d', '#a63603'), lwd = 2.5)
```
...
df_intervals$uscrs_predict <- predict(model6, newdata = df_intervals, type = 'response')
df_intervals <- df_intervals %>%
...
df_intervals_test %>%
  ...
  theme_bw() +
  scale_color_gradient(name = 'Average Medical Urgency', low = '#fdbe85', high = '#a63603') +
...
```{r, warning=F, message=F, comment=NA, fig.align='center', fig.height=8, fig.width=8}
df_intervals$uscrs_predict <- predict(model6, newdata = df_intervals)
...
df_intervals %>%
  ...
  scale_fill_manual(name = 'Status', values =c('black', '#fed976', '#feb24c', '#fd8d3c', '#f03b20', '#bd0026')) +
  geom_histogram() +  
