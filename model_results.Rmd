...
<!-- 4. Run sensitivity logistic regression models (Line 1692) -->
<!-- 5. Obtain machine learning model results (Line 1770) -->
<!-- 6. Make predictions on test dataset (Line 1832) -->
<!-- 7. Create AUC curves (Line 1893) -->
<!-- 8. Variable importance plots from gradient-boosted models (Line 2093) -->
<!-- 9. Create calibration plot in test dataset (Line 2154) -->
<!-- 10. Create facet plot of 50 point score at initial listing (Line 2213) -->
<!-- 11. Examine gender and race differences in prediction (Line 2243) -->
<!-- 12. Check for informative censoring (Line 2401) -->
<!-- 13. Examine a model with center random effects (Line 2434) -->
<!-- 14. Examine sensitivity and specificity at certain cut points (Line 2455) -->
<!-- 15. Examine model performance based on temporal split (Line 2501) -->
<!-- 16. Coefficient plot for 2 week outcome (Line 2569) -->
...
df_intervals_test_calib <- df_intervals_test_calib %>% select(death6week, uscrs_predict, uscrs_score)
...
# 14. Examine sensitivity and specificity at certain cut points for s=0
...
df_coefplot$six_stderr[df_coefplot$var == 'short_MCS_ever'] <- 
  as.data.frame(lmtest::coeftest(model_final, vcov=vcovCL, cluster=~PX_ID)[,])['short_MCS_ever','Std. Error']
df_coefplot$six_stderr[df_coefplot$var == 'log(bilirubin + 1)'] <- 
  as.data.frame(lmtest::coeftest(model_final, vcov=vcovCL, cluster=~PX_ID)[,])['log(bilirubin + 1)','Std. Error']
df_coefplot$six_stderr[df_coefplot$var == 'eGFR'] <- 
  as.data.frame(lmtest::coeftest(model_final, vcov=vcovCL, cluster=~PX_ID)[,])['eGFR','Std. Error']
df_coefplot$six_stderr[df_coefplot$var == 'sodium'] <- 
  as.data.frame(lmtest::coeftest(model_final, vcov=vcovCL, cluster=~PX_ID)[,])['sodium','Std. Error']
df_coefplot$six_stderr[df_coefplot$var == 'albumin'] <- 
  as.data.frame(lmtest::coeftest(model_final, vcov=vcovCL, cluster=~PX_ID)[,])['albumin','Std. Error']
df_coefplot$six_stderr[df_coefplot$var == 'LVAD1'] <- 
  as.data.frame(lmtest::coeftest(model_final, vcov=vcovCL, cluster=~PX_ID)[,])['LVAD1','Std. Error']
df_coefplot$six_stderr[df_coefplot$var == 'log(BNP + 1):BNP_NT_Pro0'] <- 
  as.data.frame(lmtest::coeftest(model_final, vcov=vcovCL, cluster=~PX_ID)[,])['log(BNP + 1):BNP_NT_Pro0','Std. Error']
df_coefplot$six_stderr[df_coefplot$var == 'log(BNP + 1):BNP_NT_Pro1'] <- 
  as.data.frame(lmtest::coeftest(model_final, vcov=vcovCL, cluster=~PX_ID)[,])['log(BNP + 1):BNP_NT_Pro1','Std. Error']
